var app = new Vue({
    ...
    methods: {
        displayNextQuestion: function () {
            ...
            //     this.createListOfQuestions();
            // }
            for (i = 0; i < 8; i++) {
                if (btnCollection[i].classList.contains("incorrect")) {
                    wrongAnswer = true;
                }
                btnCollection[i].classList.remove("incorrect");
            }
            if (!wrongAnswer) {
                console.log("delete:" + this.questionIndex);
                this.removeQuestionFromList(this.questionIndex);
                this.remaining = this.questionList.length;
            }
            let nextIndex = Math.floor(Math.random() * this.questionList.length);
            console.log("next index:" + nextIndex);
            console.log(this.questionList[nextIndex]);
            this.questionIndex = this.questionList[nextIndex];
            let nextResponse = this.questionBank[this.questionList[nextIndex]];
        ...
        createListOfQuestions: function () {
            for (let i = 0; i < 50; i++) {
                this.questionList.push(i);
            }
            console.log(this.questionList);
        },
        removeQuestionFromList: function (question) {
